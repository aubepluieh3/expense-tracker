 ...
 *
 * 카테고리마다 다른 색(8색 팔레트)을 주지 않는 이유:
 * 이 목록은 이모지와 이름이 이미 정체성을 담당한다. 색이 그 일을 맡지 않으면
 * 색은 다른 정보를 나를 수 있고, 그게 순위다. 무지개도 사라진다.
 *
...
export function buildSlices(rows: StatRow[]): { slices: Slice[]; rest: Slice[] } {
  ...
  const top = topRows.map((r, i) => ({
    ...
    name: r.name,
    emoji: r.emoji,
    color: rampColor(i, topRows.length),
    total: r.total,
  }))

  const rest = restRows.map((r) => ({
    id: r.category_id,
    name: r.name,
    emoji: r.emoji,
    color: OTHER_COLOR,
...
export function StackBar({ slices }: { slices: Slice[] }) {
  ...
  return (
    ...
    >
      {slices.map((s) => (
        <div key={s.id ?? 'other'} style={{ flexGrow: s.total, backgroundColor: s.color }} />
      ))}
    </div>
  )
}
